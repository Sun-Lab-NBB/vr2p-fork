    ...
    def __init__(
        ...
        if len(centers) == 0:
            ...
            self.centers = np.array([
                prop["weighted_centroid"] * np.array([1, bin_size])
                for prop in props
            ])
...
class Tank1dProtocol(PlaceFields1dProtocol):
    ...
    def detect(
        ...
        binF = convolve2d(
            binF,
            np.ones((1, self.params.smooth_size)) / self.params.smooth_size,
            mode="same",
            boundary="wrap"
        )
        ...
        pf = pf.remove_fields(
            np.argwhere(pf.max_intensity < self.params.max_int_threshold)
        )
    ...
    def validate(
        ...
        results = [
            dask.delayed(self.detect)(
                F, pos, speed, track_length, bin_size, calc_df=False
            ).has_place_field
        ]
        ...
        for i in range(num_repeats):
            ...
            res = dask.delayed(self.detect)(
                shuffled, pos, speed, track_length, bin_size, calc_df=False
            ).has_place_field
            results.append(res)
        ...
        sig_cells = np.argwhere(
            (observed) & (p < self.params.sig_threshold)
        ).flatten()
    ...
    def _shuffle(
        ...
        shuffle_ind = np.random.choice(
            np.arange(self.params.num_chunks),
...
def outside_field_threshold(
    ...
    invalid_regions = np.concatenate(
        np.argwhere(pf.mean_intensity < threshold_values)
    )
